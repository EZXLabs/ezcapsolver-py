...
from . import _http
from ._async_shortcuts import AsyncSolveShortcuts
...
__all__ = ["AsyncEzCapSolverClient"]


class AsyncEzCapSolverClient(AsyncSolveShortcuts):
    """Asynchronous EzCaptchaSolver client.

    Shares models, error types and configuration with
    :class:`~ezcapsolver.client.EzCapSolverClient`; only the I/O runs on
    ``asyncio``. One instance can serve any number of concurrent coroutines.
    ...
    Example:
        ...
        >>> async with AsyncEzCapSolverClient("your-key") as client:  # doctest: +SKIP
    ...
    def __init__(
        ...
        *,
        ...
        The arguments mean the same as on
        :class:`~ezcapsolver.client.EzCapSolverClient`; only ``http_client`` becomes
    ...
    async def aclose(self) -> None:
        """Alias of :meth:`close`, for the ``aclose`` convention.

        ``httpx`` spells it this way, and :func:`contextlib.aclosing` looks for
        exactly this name -- without it, ``async with aclosing(client)`` fails
    ...
    async def solve[S](self, task: Task[S], *, polling: PollingConfig | None = None) -> Solved[S]:
        ...
        Raises:
            ...
            SolutionDecodeError: The worker returned a shape this release does
                not model.
        """
        task_id, request_id = await self._create(task)
        try:
            result = await self._wait(task_id, polling)
    ...
    async def sync_solve[S](self, task: Task[S]) -> Solved[S]:
        ...
        Raises:
            ...
            SolutionDecodeError: The worker returned a shape this release does
                not model.
        """
        return self._solved(task, await self.create_sync_task(task))
    ...
    async def solve_raw(
        ...
        *,
        ...
        data = await self._post(url, payload, self._config.timeout)
        task_id, request_id = self._created_of(data)
        try:
            result = await self._wait(task_id, polling)
        except EzCaptchaError as exc:
            self._raise_wait_failure(exc, task_id, request_id)
    ...
    async def sync_solve_raw(self, task_type: str, params: Mapping[str, Any]) -> Solved[Any]:
        ...
        data = await self._post(url, payload, self._config.sync_timeout)
        result = self._require_terminal(TaskResult.from_dict(data))
        return Solved(
            solution=result.solution,
            raw=result.solution,
    ...
    async def create_task(self, task: Task[Any]) -> str:
        ...
        """
        return (await self._create(task))[0]

    async def create_sync_task(self, task: Task[Any]) -> TaskResult:
        ...
        result = TaskResult.from_dict(await self._post(url, payload, self._config.sync_timeout))
        _http.logger.info(
    ...
    async def get_result(self, task_id: str) -> TaskResult:
        ...
        return TaskResult.from_dict(await self._post(url, payload, self._config.timeout))

    async def wait_for_result(
        self, task_id: str, *, polling: PollingConfig | None = None
    ) -> TaskResult:
        ...
        """
        return await self._wait(task_id, polling)

    async def get_balance(self) -> float:
        """Query the account balance."""
        _http.logger.debug("querying account balance")
        url, payload = self._balance_request()
        return self._balance_of(await self._post(url, payload, self._config.timeout))

    # -- Internals --------------------------------------------------------

    async def _create(self, task: Task[Any]) -> tuple[str, str | None]:
        ...
        task_id, request_id = self._created_of(await self._post(url, payload, self._config.timeout))
        _http.logger.info("task created task_id=%s task_type=%s", task_id, task.task_type.value)
        return task_id, request_id

    async def _wait(self, task_id: str, polling: PollingConfig | None = None) -> TaskResult:
        ...
        for attempt in range(1, polling.max_attempts + 1):
            # Match the sync client: wait one interval to skip an initial processing response.
    ...
    async def _post(self, url: str, payload: dict[str, Any], timeout: float) -> dict[str, Any]:
